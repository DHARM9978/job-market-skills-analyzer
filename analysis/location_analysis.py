...
def jobs_by_location(df):
    out = (df.groupby(COL_LOCATION, observed=True)
           .agg(Listings=(COL_LOCATION,"count"), Avg_Salary=(COL_SALARY,"mean"))
           .reset_index().sort_values("Listings",ascending=False))
    # Use real lat/lon if present, else fallback to hardcoded coords
    if COL_LAT in df.columns and COL_LON in df.columns:
        ...
        out["Lat"] = out[COL_LOCATION].map(lambda c: CITY_COORDS.get(c,(None,None))[0])
        out["Lon"] = out[COL_LOCATION].map(lambda c: CITY_COORDS.get(c,(None,None))[1])
    return out
